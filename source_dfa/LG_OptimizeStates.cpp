...
int   LG_OptimizeStates::OptimizeStates ()
{
	   int rc = 0;
...
void  LG_OptimizeStates::DESTROY_LOOKAHEADS ()
{
		int s;
...
void  LG_OptimizeStates::ILLEGAL_CHARS ()
{
		int i, n, t;
		int* buffer;
...
void  LG_OptimizeStates::REMOVE_DUP_STATES ()
{
...
int   LG_OptimizeStates::REMOVE_NT_TRANS ()
{
	   int   s;
...
void  LG_OptimizeStates::LOOK_BACK (int p, int dot, int s)
{
		int i;
...
int   LG_OptimizeStates::REMOVE_OTHER_RED () // Remove NT Transitions for all lookahead actions.
{
		int s, i, t;
...
void  LG_OptimizeStates::LOOK_BACK2 (int p, int dot, int s)
{
		int i;
      if (dot-- > 0) // If not at the origin yet.
      {
...
int   LG_OptimizeStates::MODIFY_TERMINAL_TRAN ()
{
...
int   LG_OptimizeStates::NUMBER_OF_STATES_LEFT ()
{
		int i, s, nosl;
...
int   LG_OptimizeStates::COUNT_NT_LEFT ()
{
	   int h, n = 0;
		for (h = 0; h < n_heads; h++)
		{
			...
			{
				n++;
				prt_error ("Nonterminal '%s' could not be removed, lexical grammar cannot generatae a DFA",
...
int   LG_OptimizeStates::REMOVE_DUPLICATES () // Not being used 20100512
{
		int s, t, x, nd;
		nd = 0;
...
int   LG_OptimizeStates::GOESTO_CLOSURE (int s)
{
		if (s != goesto[s])
		{
			active[s] = 0;
...
void  LG_OptimizeStates::RENUMBER_STATES ()
{
      int*  newplace;
      char* replaced;
