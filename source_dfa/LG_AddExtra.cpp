...
Ret:  if (optn[LG_VERBOSE] > 2)
		prt_log ("         %7d reductions added during %d traces.\n", n_added, n_traces);
    	return (rc);
}

void	LG_AddExtra::GOTO (int s, int agoto)
{
...
void  LG_AddExtra::p_trace_stack ()
{
		int i, sym;
		char* symstr;
  		printf ("Trace stack =");
...
void 	LG_AddExtra::MERGE_NTTRANS ()
{
		int s, i, size;
		size = n_nonttran;
      n_nonttran = 0;
...
void  LG_AddExtra::LOOK_BACK (int p, int dot, int x, int& nj, int* jumpto)
{
      if (dot-- > 0)
      {
...
int 	LG_AddExtra::RECURSIVE (int h)
{
      int p, t, sym;
      if (!onstack[h])
      {
...
int 	LG_AddExtra::POSSIBIL (int h)
{
...
void  LG_AddExtra::p_prod (int p, int dot)
{
      int t, u, d;
...
void  LG_AddExtra::PRT_STA (int s)
{
      int k, i, f, p, n, a, b;
